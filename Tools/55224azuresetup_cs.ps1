...
### Test Network Connection every minute to avoid Azure Cloud Shell 20 minute timeout
$Increment = 0 ; 
$TestNetworkJob = Start-Job –Name TestNetwork –ScriptBlock { Do {Test-NetConnection ; Start-Sleep 60} While ($args[0] -eq 0) } -ArgumentList @($Increment)
...
$VMSize = (Get-AzureRMVMSize -Location $Location | Where-Object {$_.Name -like "Standard_DS2*"})[-1].Name
...
Rename-Item -Path $WorkFolder$logFilePrefix$logFileSuffix -NewName $WorkFolder"55224AzureSetup"$et$logFileSuffix
